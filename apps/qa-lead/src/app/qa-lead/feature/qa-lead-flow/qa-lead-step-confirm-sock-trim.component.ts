...
@Component({
  selector: 'app-qa-lead-step-confirm-sock-trim',
  standalone: true,
  imports: [
    CommonModule,
    NzGridModule,
...
export class QaLeadStepConfirmSockTrimComponent {
  qaLeadStore = inject(QaLeadStore);
  stepConfirmSockTrim = this.qaLeadStore.$currentScanStep as Signal<QaScanItemApi.QaScanStep>; // cast non null bcz not nullable in here
  ...
  scan() {
    ...
    if (reviewBarcode) {
      this.qaLeadStore.scanConfirmSockTrim(scanValue);
    } else {
