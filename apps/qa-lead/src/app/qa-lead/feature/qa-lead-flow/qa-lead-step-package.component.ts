...
@Component({
  selector: 'app-qa-lead-step-package',
  standalone: true,
  imports: [
    CommonModule,
    NzGridModule,
    ...
    <div class="tw-flex tw-flex-col tw-h-full ">
      ...
      <div class="tw-flex tw-gap-2 tw-flex-1">
        <app-scan-board color="white" [label]="'PACKAGE' | translate" class="tw-flex-1">
          <div class="tw-h-full tw-flex tw-flex-wrap tw-items-center tw-justify-center tw-gap-y-2">
            <div
              *ngFor="let onePackage of $stepPackage().scanningPackages!.packages"
              ...
              <img [src]="onePackage.imageUrl" class="tw-w-full tw-max-h-80 tw-object-contain" appPreviewImage appImageErrorUrl />
            </div>
          </div>
        </app-scan-board>
      </div>
...
export class QaLeadStepPackageComponent {
  qaLeadStore = inject(QaLeadStore);
  $stepPackage = this.qaLeadStore.$currentScanStep as Signal<QaScanStep>; // cast non null bcz not nullable in here

  scanPackageControl = new FormControl('', { nonNullable: true });

  ngOnInit() {
    if (this.qaLeadStore.$isPrintCustomInsert()) {
      this.qaLeadStore.printCustomInsert();
    }
  }
  ...
  scanPackage() {
    ...
    if (onePackage) {
      this.qaLeadStore.scanPackage({ packageId: onePackage.id });
    } else {
