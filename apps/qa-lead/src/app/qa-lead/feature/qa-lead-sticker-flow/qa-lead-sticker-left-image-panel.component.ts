...
@Component({
  selector: 'app-qa-lead-sticker-left-image-panel',
  standalone: true,
...
export class QaLeadStickerLeftImagePanelComponent {
  qaLeadStickerStore = inject(QaLeadStickerStore);
  $previewImages: Signal<QaPreviewImage[]> = this.qaLeadStickerStore.selectSignal(state => {
  ...
  handleStepGroupBarcodeImage(images: QaPreviewImage[], step: QaStickerScanSheetApi.QaStickerStepStickerGroup) {
    if (this.qaLeadStickerStore.$currStickerGroup()?.scanningStickerGroup.barcode === step.scanningStickerGroup.barcode) return;
