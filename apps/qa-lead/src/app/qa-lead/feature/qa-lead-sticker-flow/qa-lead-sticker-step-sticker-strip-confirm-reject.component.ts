...
@Component({
  selector: 'app-qa-lead-sticker-step-sticker-strip-confirm-reject',
  standalone: true,
  imports: [
    CommonModule,
  ...
  template: `
    <div class="tw-flex tw-flex-col tw-h-full">
      <!-- Qty -->
      <div class="tw-flex tw-gap-6 tw-mb-1">
        <div class="tw-w-[300px] tw-flex tw-items-center tw-justify-end">
          <label for="qty-input" class="tw-font-semibold tw-text-xl">{{ 'QA.QUANTITY' | translate | uppercase }}</label>
        </div>
        <div class="tw-flex-1">
...
export class QaLeadStickerStepStickerStripConfirmRejectComponent {
  ...
  scan() {
    ...
    const matchingConfirmBarcode = this.qaLeadStickerStore
      .selectSignal(s => s.sheet!.qaLeadRejectReviewBarcodes)()
      .find(rb => rb.barcode === scanValue);
    if (!matchingConfirmBarcode) {
    ...
    this.qaLeadStickerStore.confirmReject({ confirmBarcode: matchingConfirmBarcode });
  }
