...
@Component({
  selector: 'app-qa-lead-sticker-step-package',
  standalone: true,
  imports: [
    CommonModule,
...
export class QaLeadStickerStepPackageComponent {
  qaLeadStickerStore = inject(QaLeadStickerStore);
  $stepPackage = this.qaLeadStickerStore.$currentScanStep as Signal<QaStickerScanSheetApi.QaStickerStepPackage>;
  ...
  scanPackage() {
    ...
    if (matchingPackage) {
      this.qaLeadStickerStore.scanPackage({ scannedPackage: matchingPackage });
    } else {
