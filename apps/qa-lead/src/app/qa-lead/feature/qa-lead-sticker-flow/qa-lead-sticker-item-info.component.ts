...
import { QaItemGeneralInfo } from '../../data-access/model/ui/qa-lead-item-general-info';

@Component({
  selector: 'app-qa-lead-sticker-item-info',
  standalone: true,
...
export class QaLeadStickerItemInfoComponent {
  qaLeadStickerStore = inject(QaLeadStickerStore);
  $itemInfo: Signal<Omit<QaItemGeneralInfo, 'sku'> | null> = this.qaLeadStickerStore.selectSignal(state => {
  ...
  $isCurrStepAfterStepCoo = this.qaLeadStickerStore.selectSignal(s => {
  ...
  resetCoo() {
    this.qaLeadStickerStore.patchState(s => {
