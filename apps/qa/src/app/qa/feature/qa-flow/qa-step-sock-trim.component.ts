...
@Component({
  selector: 'app-qa-step-sock-trim',
  standalone: true,
  imports: [
    CommonModule,
    NzGridModule,
...
export class QaStepSockTrimComponent {
  qaStore = inject(QaStore);
  stepSockTrim = this.qaStore.$currentScanStep as Signal<QaScanItemApi.QaScanStep>; // cast non null bcz not nullable in here
  ...
  scan() {
    ...
    if (scanValue === insert.barcode.toUpperCase()) {
      this.qaStore.scanSockTrim(insert.id);
    } else {
