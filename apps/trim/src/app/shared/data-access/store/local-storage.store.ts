...
export interface TrimConfigKey {
...
export class LocalStorageStore extends ComponentStore<LocalStorageState> {
  ...
  //region Trim
  trimConfigKey = 'trim-config';
  trimConfig$ = this.select(state => state.trimConfig);
  getTrimConfig() {
    return lsGetKey<TrimConfigKey>(this.trimConfigKey);
  }
  setTrimConfig(value: { factory: UserFactoriesGetApi.ResponseItem | null; station: StationsGetApi.ResponseItem | null }) {
    const notNullValue: TrimConfigKey = {
      factory: value.factory!,
      station: value.station!,
    };
    lsSetKey(this.trimConfigKey, notNullValue);
    this.patchState({ trimConfig: notNullValue });
  }
  removeTrimConfig() {
    lsRemoveKey(this.trimConfigKey);
    this.patchState({ trimConfig: null });
  }
  //endregion
